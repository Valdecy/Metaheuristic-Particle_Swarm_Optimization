...
def update_position(position, velocity, min_values = [-5,-5], max_values = [5,5], target_function = target_function):
    ...
    for i in range(0, position.shape[0]):
        for j in range(0, position.shape[1] - 1):
            updated_position[i,j] = np.clip((position[i,j] + velocity[i,j]),  min_values[j],  max_values[j])
...
pso = particle_swarm_optimization(swarm_size = 15, min_values = [-5,-5], max_values = [5,5], iterations = 100, decay = 2, w = 0.9, c1 = 2, c2 = 2, target_function = six_hump_camel_back)
...
pso = particle_swarm_optimization(swarm_size = 50, min_values = [-5,-5], max_values = [5,5], iterations = 100, decay = 2, w = 0.9, c1 = 2, c2 = 2, target_function = rosenbrocks_valley)
